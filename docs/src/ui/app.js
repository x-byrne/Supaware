...
export class App {
  ...
  async _renderReturnsChart(from, to) {
    ...
    const datasets = [];
    const labels = [];
    const returnsBySeries = {};

    for (const id of ids) {
      const meta = this.catalog.get(id);
      const rows = allData[id].filter(r => r.period >= from && r.period <= to);
      if (rows.length < 2) continue;
      const rebased = rows.map((r, i) => ({
        ...r,
        value: ((r.value / rows[0].value) - 1) * 100
      }));
      returnsBySeries[meta.name] = rebased;
      const totalReturn = rebased[rebased.length - 1].value;
      const annualReturn = ((Math.pow(1 + totalReturn / 100, 365 / (rows[rows.length - 1].period - rows[0].period) * 86400000) - 1) * 100);
      labels.push(meta.name);
      datasets.push({
        label: meta.name,
        data: [parseFloat(totalReturn.toFixed(2))],
        backgroundColor: PALETTE[ids.indexOf(id) % PALETTE.length] + '33',
        borderColor: PALETTE[ids.indexOf(id) % PALETTE.length],
        borderWidth: 2,
        borderRadius: 4
      });
    }
